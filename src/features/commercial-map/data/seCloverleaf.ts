...
const profile = REGIONAL_HIGHWAY_PROFILE;
/** Cap the folded 6.4 ribbon; pick up the thinned regional carriageway when present. */
const highwayWidth = Math.min(profile.carriagewayWidth, 1.52);
const highwayShoulder = Math.min(profile.shoulderWidth, 0.34);
