...
const OUT = path.join(ROOT, "docs/validation/road-precision");
const args = process.argv.slice(2);
const url = args[1] || "http://127.0.0.1:4186/mapa-comercial";
...
async function main() {
  ...
  if (!(await search.isVisible())) await page.getByRole('button',{name:/^(Buscar no mapa comercial|Abrir busca do mapa, filtro ativo)$/}).click();
  await search.fill('Q-R-15');
  await search.press('Enter');
  await page.getByRole('option',{name:/Q-R-15/}).click();
  await page.waitForTimeout(2200);
  result.checks.preservedLotSelectable = await page.evaluate(async()=>{
    const s=(await import('/src/features/commercial-map/state/useCommercialMapStore.ts')).useCommercialMapStore.getState();
    return s.selectedEntityId?.toLowerCase().endsWith('q-r-15');
  });
  await page.screenshot({path:path.join(OUT,`functional-${mobile?'mobile':'desktop'}-lot-r15.png`)});
  // Exercise environmental/segment presentation on the mounted full map.
  await page.evaluate(async()=>{
    const s=(await import('/src/features/commercial-map/state/useCommercialMapStore.ts')).useCommercialMapStore.getState();
    s.requestSegmentFocus('exporural');s.setLabelsVisible(false);
  });
  await page.waitForTimeout(2000);
  result.checks.segmentAndLabels = await page.evaluate(async()=>{
    const s=(await import('/src/features/commercial-map/state/useCommercialMapStore.ts')).useCommercialMapStore.getState();
    const valid=s.activeSegmentId==='exporural'&&!s.labelsVisible;
    s.clearSegmentFocus();s.setLabelsVisible(true);s.inspectParkingBlock(null);
    return valid;
  });
  await page.getByRole('complementary',{name:'Inspeção do estacionamento posterior',exact:true}).waitFor({state:'visible',timeout:15000});
  result.checks.parkingInspection = true;
  await page.screenshot({path:path.join(OUT,`functional-${mobile?'mobile':'desktop'}-parking.png`)});
  await page.getByRole('button',{name:'Fechar inspeção do estacionamento',exact:true}).click();
  await page.waitForTimeout(2200);
