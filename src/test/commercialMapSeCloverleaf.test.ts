...
import { INTERCHANGE_ENVELOPES } from '@/features/commercial-map/data/regional-highways';
import {
...
describe('trevo sul BR-472 — isolamento', () => {
  ...
  it('preserva o trevo em Y do Portão 5 byte a byte', () => {
    ...
    expect(REAR_CALIBRATED_AXES.br472SouthRampToSouth.at(-1)).toEqual(SE_CLOVERLEAF_PUBLISHED_JOIN_SOURCE);
  });
});
...
describe('trevo sul BR-472 — implantação', () => {
  it('encaixa no término sul da BR-472, a sudeste do parque e a sul do A5', () => {
    ...
    expect(SE_CLOVERLEAF_JOIN_SOURCE).not.toEqual([6146, 4400]);
    expect(SE_CLOVERLEAF_CENTER_LOCAL[0]).toBeCloseTo(INTERCHANGE_ENVELOPES.seCloverleaf.center[0], 6);
    expect(SE_CLOVERLEAF_CENTER_LOCAL[1]).toBeCloseTo(INTERCHANGE_ENVELOPES.seCloverleaf.center[1], 6);
    expect(SE_CLOVERLEAF_JOIN_LOCAL[0]).toBeCloseTo(SE_CLOVERLEAF_CENTER_LOCAL[0], 6);
