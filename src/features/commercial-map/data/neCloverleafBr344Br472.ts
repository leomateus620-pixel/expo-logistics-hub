...
/**
 * Dual-carriageway family. Pick up the thinned regional profile when present
 * (`dualCarriagewayWidth` / `medianWidth`); otherwise keep Anexo 2 ribbon
 * sizes so this slice still compiles against the folded 6.4 mainline profile.
 */
type RegionalProfileFields = typeof REGIONAL_HIGHWAY_PROFILE & {
  dualCarriagewayWidth?: number;
  medianWidth?: number;
};

const profile = REGIONAL_HIGHWAY_PROFILE as RegionalProfileFields;
const dualCarriagewayWidth = profile.dualCarriagewayWidth ?? 0.7;
const medianWidth = profile.medianWidth ?? 0.13;
const shoulderWidth = Math.min(profile.shoulderWidth, 0.34);
const edgeLineWidth = Math.min(profile.edgeLineWidth, 0.05);
...
export const NE_CLOVERLEAF_LAYOUT = Object.freeze({
  ...
  medianWidth,
  carriagewayWidth: dualCarriagewayWidth,
  shoulderWidth,
  edgeLineWidth,
  stubLength: 12,
  ...
  quadrantOffset: 3.05,
  innerRampWidth: dualCarriagewayWidth,
  innerRampShoulder: shoulderWidth * 0.72,
  outerRampWidth: dualCarriagewayWidth * 0.96,
  outerRampShoulder: shoulderWidth * 0.68,
  loopRadius: 5.05,
  goreGap: 0.055,
  goreLength: 1.18,
