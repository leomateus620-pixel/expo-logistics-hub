...
const out = path.resolve("docs/validation/pole-location", phase);
(async () => {
  ...
  try {
    await page.goto(
      "http://127.0.0.1:4201/mapa-comercial?groundQa&quality=fixed",
      { timeout: 120000 },
    );
    ...
    const poses = {
      arenaEvents: pose([24,2],[12,8,12]),
      arenaEventsGround: pose([22,3],[8,2.5,5]),
      arenaTop: pose([30,0],[0.01,24,0]),
      p10: pose(center(find("Q-P-10")), [6,3.4,5]),
      p10Ground: pose(center(find("Q-P-10")), [5,1,1]),
      cooperativism: pose(center(find("B28")), [7,3,5]),
      alameda: pose(center(find("D1")), [7,4,5]),
      north: pose([0,-25],[0.01,50,0]),
      center: pose([-7,0],[0.01,37,0]),
      south: pose([0,22],[0.01,32,0]),
      headquarters: pose(center(find("B12")),[-3,4,-4]),


      medium: pose([-10,0],[0,30,25]),
      r2: pose(r2,[4,3,5]),
    ...
    if (process.argv.includes("--verify")) {
      ...
      report.selections = [];
      for (const id of ["Q-P-10", "Q-M-08", "B28", "D1"]) {
