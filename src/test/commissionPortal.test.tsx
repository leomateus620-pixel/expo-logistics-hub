...
describe("CommissionPortalPage", () => {
  ...
  it("presents one primary headline and separates every registry state", () => {
    ...
    const activeGroup = screen.getByRole("region", {
      name: "Disponível agora",
    });
    const structuringGroup = screen.getByRole("region", {
      name: "Em estruturação",
    });
    const restrictedGroup = screen.getByRole("region", {
    ...
    expect(
      screen.getByRole("heading", { name: "Disponível agora" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Em estruturação" }),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("button", { name: "Acessar área administrativa" }),
    ).toBeInTheDocument();
  });
  ...
  it("preserves the Admin and Cronograma login destinations and selected context", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Acessar Cronograma e Eventos" }),
    );
    expect(screen.getByTestId("current-location")).toHaveTextContent(
      "/login/cronograma-eventos",
    );
    expect(localStorage.getItem(SELECTED_COMMISSION_STORAGE_KEY)).toBe(
      "cronograma-eventos",
    );
  });

  it("opens Eventos Restaurante e Arena as an independent authenticated destination", () => {
    renderPortal();

    fireEvent.click(
      screen.getByRole("button", {
        name: "Acessar Eventos Restaurante e Arena",
      }),
    );

    expect(screen.getByTestId("current-location")).toHaveTextContent(
      "/login/eventos-restaurante-arena",
    );
    expect(localStorage.getItem(SELECTED_COMMISSION_STORAGE_KEY)).toBe(
  ...
  it("keeps commission expansion, Escape dismissal, and module login routing", () => {
    ...
    expect(
      screen.getByRole("region", { name: "Detalhes de Logística" }),
    ).toBeInTheDocument();

    fireEvent.keyDown(window, { key: "Escape" });
    expect(logisticsToggle).toHaveAttribute("aria-expanded", "false");
    expect(
      screen.queryByRole("region", { name: "Detalhes de Logística" }),
    ).not.toBeInTheDocument();

    fireEvent.click(logisticsToggle);
    fireEvent.click(
      screen.getByRole("button", { name: "Acessar módulo Logística" }),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Acessar módulo Gastronomia" }),
    );
    expect(screen.getByTestId("current-location")).toHaveTextContent(
      "/login/gastronomia",
    );
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "Acessar módulo Financeiro Gerencial",
      }),
    );
    expect(screen.getByTestId("current-location")).toHaveTextContent(
      "/login/financeiro-gerencial",
    );
    expect(localStorage.getItem(SELECTED_COMMISSION_STORAGE_KEY)).toBe(
