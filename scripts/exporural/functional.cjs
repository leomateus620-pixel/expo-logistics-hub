...
const OUT = path.join(ROOT, "docs/screenshots/exporural-upgrade");
...
async function main() {
  ...
  const snapshots=[result.before,result.selected,result.interior,result.landscape,result.after].filter(Boolean);
  ...
  await browser.close();
  const required=['navigation','filtersOpened','filterToggled','selection','interior','interiorExit','singleCanvas','renderHealth','noOverflow','noBackendWrites','noPageErrors'];
