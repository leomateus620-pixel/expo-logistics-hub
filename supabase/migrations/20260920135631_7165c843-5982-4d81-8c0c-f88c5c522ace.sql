-- =====================================================================
-- Mapa Comercial público: links por área, leitura pública e telemetria
-- =====================================================================

CREATE TABLE public.public_map_links (
...
-- Sem policies: todo acesso acontece por funções SECURITY DEFINER validadas.

CREATE TABLE public.public_map_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  event_type text NOT NULL CHECK (event_type IN (
    'area_visit', 'map_ready', 'lot_selected', 'lot_details_viewed', 'engagement_interval', 'viewer_error'
  )),
...
CREATE INDEX public_map_events_link_time_idx ON public.public_map_events (link_id, occurred_at DESC);
CREATE INDEX public_map_events_lot_idx ON public.public_map_events (lot_id) WHERE lot_id IS NOT NULL;
CREATE INDEX public_map_events_session_idx ON public.public_map_events (session_id);
...
-- ---------------------------------------------------------------------
-- Resolução de escopo (sempre por UUID canônico)
-- ---------------------------------------------------------------------
...
REVOKE EXECUTE ON FUNCTION public.public_map_scope_entity_ids(uuid) FROM PUBLIC, anon, authenticated;

-- ---------------------------------------------------------------------
-- Validação de token
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.public_map_resolve_link(_slug text, _token text)
RETURNS public.public_map_links
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_link public.public_map_links;
BEGIN
...
REVOKE EXECUTE ON FUNCTION public.public_map_resolve_link(text, text) FROM PUBLIC, anon, authenticated;

-- ---------------------------------------------------------------------
-- Disponibilidade publicável (nunca expõe reserva/negociação nominal)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Ficha pública de um lote (valida escopo do lote)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Telemetria (única escrita externa)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Gestão (capability map.analytics.view)
-- ---------------------------------------------------------------------
...
REVOKE EXECUTE ON FUNCTION public.public_map_require_manager(uuid) FROM PUBLIC, anon;
...
CREATE OR REPLACE FUNCTION public.public_map_interest_summary(_from timestamptz DEFAULT now() - interval '30 days', _to timestamptz DEFAULT now())
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_org uuid;
...
-- ---------------------------------------------------------------------
-- Seed dos 10 escopos (inativos, sem token)
-- ---------------------------------------------------------------------
DO $seed$
DECLARE
  v_project uuid;
  ...
  v_auto uuid[];
BEGIN
  SELECT id, org_id INTO v_project, v_org FROM public.map_projects ORDER BY created_at LIMIT 1;
  IF v_project IS NULL THEN RETURN; END IF;

  SELECT array_agg(e.id) INTO v_auto
    FROM public.map_entities e
   ...
   WHERE e.project_id = v_project
     ...
     AND l.block IN ('O', 'P', 'Q', 'T', 'U', 'V');

  INSERT INTO public.public_map_links (org_id, project_id, slug, display_name, scope_kind, scope_key, scope_entity_ids, sort_order)
  VALUES
    (v_org, v_project, 'pavilhao-1', 'Pavilhão 1', 'PAVILION', 'B1', '{}', 10),
    (v_org, v_project, 'pavilhao-3', 'Pavilhão 3', 'PAVILION', 'B6', '{}', 20),
    (v_org, v_project, 'pavilhao-5', 'Pavilhão 5', 'PAVILION', 'B8', '{}', 30),
    (v_org, v_project, 'pavilhao-8', 'Pavilhão 8', 'PAVILION', 'B4', '{}', 40),
    (v_org, v_project, 'pavilhao-12', 'Pavilhão 12', 'PAVILION', 'B3', '{}', 50),
    (v_org, v_project, 'pavilhao-13', 'Pavilhão 13', 'PAVILION', 'B5', '{}', 60),
    (v_org, v_project, 'pavilhao-14', 'Pavilhão 14', 'PAVILION', 'B2', '{}', 70),
    (v_org, v_project, 'exporural', 'Exporural', 'SEGMENT', 'exporural', '{}', 80),
    (v_org, v_project, 'industria-comercio-servicos-externo', 'Indústria, Comércio e Serviços — área externa', 'SEGMENT_EXTERNAL', 'industria-comercio-servicos', '{}', 90),
    (v_org, v_project, 'espaco-automovel', 'Espaço do Automóvel', 'ENTITY_SET', NULL, coalesce(v_auto, '{}'), 100)
