...
const PERMANENT_SAMPLE = [
  'B11', 'B12', 'B15', 'B19', 'B28', 'B29', 'B37', 'B41', 'B42-01', 'B42-02',
...
describe('limpeza estrutural dos blocos não permanentes', () => {
  it('remove exatamente 41 identificadores (26 sanitários + 15 estruturas)', () => {
    const restrooms = NON_PERMANENT_REMOVED_IDENTIFIERS_2026.filter((id) => id.startsWith('E-'));
    expect(restrooms).toHaveLength(26);
    expect(NON_PERMANENT_REMOVED_IDENTIFIERS_2026).toHaveLength(41);
    expect(new Set(NON_PERMANENT_REMOVED_IDENTIFIERS_2026).size).toBe(41);
  });
