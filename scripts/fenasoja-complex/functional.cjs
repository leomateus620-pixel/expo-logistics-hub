...
const OUT = path.join(ROOT, process.env.QA_OUTPUT || "docs/screenshots/fenasoja-complex");
const args = process.argv.slice(2);
const url = args[1] || "http://127.0.0.1:4194/mapa-comercial";
...
async function main() {
  ...
  const postponePush=page.getByRole('button',{name:'Agora não',exact:true});
  if(await postponePush.count()) await postponePush.click();
  ...
  await page
    .getByRole("searchbox", { name: "Buscar no mapa comercial", exact: true })
    .press("Enter");
  await page.getByRole("option", { name: /B12\./ }).click();
  await page.waitForTimeout(2500);
  result.checks.selection = await page.locator("[data-commercial-map-interior-trigger]").count() > 0;

  result.selected = await snapshot(page); await page.screenshot({path:path.join(OUT,`functional-${mobile ? "mobile" : "desktop"}-B12-front.png`)});
  ...
  for (const id of ['B12','B13','B12','B13']) {
    const search = page.getByRole('searchbox', {name:'Buscar no mapa comercial', exact:true});
    if (!(await search.isVisible())) await page.getByRole('button', {name:'Buscar no mapa comercial',exact:true}).click();
  ...
  const dismiss = page.getByRole('button', {name:'Fechar painel',exact:true});
  if(await dismiss.count()) await dismiss.first().click();
  if(mobile) { await page.setViewportSize({width:390,height:844}); await page.waitForTimeout(800); }
  const currentBox = await canvas.boundingBox();
  await page.mouse.move(currentBox.x+currentBox.width*.45,currentBox.y+currentBox.height*.32);
  const beforePan = await snapshot(page);
  await page.mouse.down({button:'right'}); await page.mouse.move(currentBox.x+currentBox.width*.48,currentBox.y+currentBox.height*.34,{steps:8}); await page.mouse.up({button:'right'}); await page.waitForTimeout(600);
  result.checks.pan = JSON.stringify((await snapshot(page)).camera?.target)!==JSON.stringify(beforePan.camera?.target);
  result.extremeZoom = [];
  for (const direction of [-1,1]) {
    for(let i=0;i<20;i++) { await page.mouse.wheel(0,direction*1000); await page.waitForTimeout(60); }
    await page.waitForTimeout(700); result.extremeZoom.push(await snapshot(page));
  }
  if (mobile) {
    const cdp = await page.context().newCDPSession(page), x=currentBox.x+currentBox.width*.4, y=currentBox.y+currentBox.height*.32;
    const beforePinch = await snapshot(page);
    await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:x-30,y,id:1},{x:x+30,y,id:2}]});
    for(let i=1;i<=8;i++) await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:x-30-i*5,y,id:1},{x:x+30+i*5,y,id:2}]});
    await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]}); await page.waitForTimeout(700);
    result.checks.emulatedPinch = JSON.stringify((await snapshot(page)).camera?.position)!==JSON.stringify(beforePinch.camera?.position);
  }
  result.checks.newStructuresSelectable = result.checks.infrastructureSelections.every(s=>s.correct);
  result.after = await snapshot(page);
  const snapshots=[result.before,result.selected,result.interior,result.landscape,result.after,...result.extremeZoom].filter(Boolean);
  result.checks.singleCanvas=snapshots.every(s=>s.canvasCount===1);
  result.checks.renderHealth=snapshots.every(s=>s.health?.status==='ready' && s.health.contextLosses===0 && s.health.lastErrorCode===null);
