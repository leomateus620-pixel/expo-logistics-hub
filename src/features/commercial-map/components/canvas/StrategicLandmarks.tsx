...
import { FenasojaHeadquarters } from './FenasojaHeadquarters';
import { FenasojaComplexOverlay } from './FenasojaComplexOverlay';
  ...
  return (
    ...
    >
      ...
      <group rotation={[0, facingRadians, 0]} dispose={null}>
        {import.meta.env.DEV && (entity.publicIdentifier === 'B12' || entity.publicIdentifier === 'B13') && new URLSearchParams(window.location.search).has('complexDebug') && <FenasojaComplexOverlay identifier={entity.publicIdentifier} bounds={bounds} />}
        {kind === 'soy-restroom' && <SoyRestroom materials={materials} />}
        {kind === 'gate-nine-tanks' && <GateNineTanks bounds={bounds} materials={materials} />}
        {kind === 'administrative-center' && <AdministrativeCenter {...modelProps} />}
        {kind === 'fenasoja-headquarters' && <FenasojaHeadquarters bounds={bounds} toneDown={toneDown} showDetail={showDetail} />}
        {kind === 'lactalis-cultural-stage' && (
