...
A prévia revelou um caso específico de Fast Refresh: React preservava um material, mas o recarregamento do módulo reiniciava seu WeakSet e permitia injetar GLSL uma segunda vez (`direct-shader-failed`). A instalação agora marca a própria função com um Symbol global, conservando a idempotência entre versões do módulo. Um teste que reinicia o cache de módulos mantendo o material confirma shader e texturas idênticos; os sete testes do padrão canônico passaram (`hot-reload-tests.json`). A validação real no navegador integrado executou duas alterações de código seguidas e zoom, mantendo `ready`/`post`, 365 quadros apresentados, zero perdas de contexto e `lastErrorCode: null` (`hot-reload-runtime.json`). Esse ajuste não altera o GLSL de uma instalação normal; as capturas e medições anteriores continuam representando a apresentação final.
