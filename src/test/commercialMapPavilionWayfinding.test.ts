...
describe('orientação visual das plantas internas comerciais', () => {
  ...
  it('corrige o Pavilhão 13 e ancora entradas, saídas e conexões nos seis vãos oficiais', () => {
    ...
    const southwestPoint = projectCommercialPavilionReferencePoint([6.6 / 19.8, 1], frame);
    const southeastPoint = projectCommercialPavilionReferencePoint([14.7 / 19.8, 1], frame);
  ...
  it('não intercepta lotes e descarta recursos GPU ao trocar de interior', () => {
    ...
    expect(canvas).toContain('const currentInterior = useCommercialMapStore.getState().interiorEntityId');
    expect(canvas).toContain('targetEntityId === currentInterior');
    expect(canvas).toContain('<InteriorCameraRequestContext.Provider');
